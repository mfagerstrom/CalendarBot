...
const buildGroceryListData = (
  ...
  for (const item of completedItems) {
    ...
    section.completedItems.push(text);
...
const buildGroceryListComponents = (
  ...
  if (data) {
    if (data.sourceUpdatedAt) {
      const sourceUpdated = new Date(data.sourceUpdatedAt);
      if (!Number.isNaN(sourceUpdated.getTime())) {
        overviewContainer.addTextDisplayComponents(
          new TextDisplayBuilder().setContent(
            `-# Source updated: ${formatTimestampEt(sourceUpdated)}`,
          ),
        );
      }
    }
